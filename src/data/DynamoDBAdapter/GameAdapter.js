...
export default class extends DynamoDBAdapter {
  ...
  async getGameFromFile(gameId, teamSets = null) {
    ...
    await Promise.all(game.state.teams.filter(t => !!t).map(async team => {
      ...
      if (teamSets === null) {
        ...
        const teamSetStats = teamSets.get(team.set.key)?.stats ?? TeamSetStats.create();
        teamSetStats.id = team.set.id;
        team.set.stats = teamSetStats;
        await this._getTeamSetStatsPlayer(team.set, team.playerId);
  ...
  _getGameSummaryListIds(game) {
    ...
    if (game.state.rated && game.state.endedAt) {
      ...
      if (
        ...
        game.state.teams.every(t => t.set.isFull)
  ...
  _getTeamSetById(gameType, teamSetId)  {
  ...
  _getTeamSet(teamSetData, gameType, teamSetId = undefined) {
    ...
    teamSet.on('stats:change:rating', () => this.buffer.get('teamSetIndex').add(`${teamSet.key}:rating`, {
      metricName: 'rating',
      teamSet,
    }));
    teamSet.on('stats:change:gameCount', () => this.buffer.get('teamSetIndex').add(`${teamSet.key}:gameCount`, {
      metricName: 'gameCount',
      teamSet,
    }));
    teamSet.on('stats:change:playerCount', () => this.buffer.get('teamSetIndex').add(`${teamSet.key}:playerCount`, {
  ...
  async _saveTeamSetStats(teamSet, reset = false) {
    ...
    if (reset) {
      ...
      this.buffer.get('teamSetIndex').add(`${teamSet.key}:rating`, {
        metricName: 'rating',
        teamSet,
      });
      this.buffer.get('teamSetIndex').add(`${teamSet.key}:gameCount`, {
        metricName: 'gameCount',
        teamSet,
      });
      this.buffer.get('teamSetIndex').add(`${teamSet.key}:playerCount`, {
        metricName: 'playerCount',
        teamSet,
      });
    }
