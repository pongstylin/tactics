...
export default class extends DynamoDBAdapter {
  ...
  async forkGame(game, clientPara, options) {
    ...
    if (options.turnId === undefined)
      options.turnId = this.state.currentTurnId;
    if (options.turnId > this.state.currentTurnId)
      options.turnId = this.state.currentTurnId;
  ...
  async _getGame(gameId, withRecentTurns = true) {
    ...
    if (!game.state.teams.some(t => !!t)) {
      ...
      // A game is usually loaded with the current turn, which is required to view the game.
      if (game.state.startedAt && withRecentTurns)
  ...
  async _getGameRecentTurns(game) {
    ...
    for (let turnId = game.state.currentTurnId; turnId >= game.state.initialTurnId; turnId--) {
      const turn = await this._getGameTurn(game, turnId);

      // Don't count the current turn.
