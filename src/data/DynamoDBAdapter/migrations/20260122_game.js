...
export default async function (itemMap) {
  ...
  const startedAt = item.D.$data.state.startedAt;

  for (const teamItem of teamsItem.values()) {
    const set = teamItem.D.$data.set;
    if (!set || !set.units) continue;

    if (teamItem.D.$data.setVia === 'temp' && startedAt < '2026-01-19T:00:00:00')
      teamItem.D.$data.setVia = 'default';
