...
export default class Game extends ActiveModel {
  ...
  fork(clientPara, { turnId, vs, as, timeLimitName }) {
    ...
    const forkGame = Game.create({
      ...
      autoSurrender: false,
      rated: false,
      teams: new Array(this.state.teams.length).fill(null),
    });
    ...
    if (forkGame.state.timeLimit)
      forkGame.state.currentTurn.timeLimit = getTurnTimeLimit[forkGame.state.timeLimit.type].call(forkGame.state);
