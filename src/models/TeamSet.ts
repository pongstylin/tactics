...
export default class TeamSet extends ActiveModel {
  ...
  private _indexPaths: ReturnType<TeamSetCardinality['getIndexPaths']> | null = null;
  ...
  get indexPaths() {
    return this._indexPaths ??= this.cardinality.getIndexPaths(this);
  }
  ...
  _generateName() {
    ...
    const nameParts = [
      (() => {
        const units:{ name:string, count:number, rarity:number, tagIndex:any }[] = [];
        for (const tag of tags) {
          if (tag.type !== 'unit') continue;
          if (tag.name === 'sg' && tag.count === undefined && tags.some(t => t.type === 'type' && t.name === 'turtle')) continue;
          ...
          units.push({
            ...
            rarity: unitTypeByCode.get(tag.name).rarity,
            tagIndex,
          });
        }
        if (units.length === 0) return null;
        const unit = units.sort((a,b) => a.count - b.count || b.rarity - a.rarity)[0];
        if (
