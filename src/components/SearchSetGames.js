...
export default class SearchSetGames {
  ...
  async renderGames() {
    ...
    const divGames = await Promise.all(this.result.gamesSummary.map(gs => gameCard.renderGame(gs, { setId:this.set.id })));
  ...
  async selectVSSet() {
    ...
    if (choice === 'View Set')
      ...
      this.els.vsSet.component.reset(this.gameType, { name:'Select Set...' });
