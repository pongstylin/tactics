...
import SetImageRenderer from 'components/SetImageRenderer.js';
...
window.Tactics = {
  ...
  editSet: async function (data) {
    ...
    const setBuilder = await this.setBuilder;
    setBuilder.gameType = data.gameType;
    setBuilder.set = data.set;
    return setBuilder.show();
  },
  draw: function (data) {
    const types = new Map([
      [ 'C', PIXI.Container ],
